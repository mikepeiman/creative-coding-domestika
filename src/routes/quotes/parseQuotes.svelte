...
<script>
	...
	function parseQuoteAuthorName(workingQuoteObject, separatorValue) {
		...
		if (separatorValue) {
			...
			console.log(
				`🚀 ~ file: parseQuotes.svelte ~ line 238 ~ parseQuoteAuthorName ~ author`,
				author
			);
			remainder = Array.from(item).splice(separatorValue, itemEnd).join(String());
			console.log(
				`🚀 ~ file: parseQuotes.svelte ~ line 240 ~ parseQuoteAuthorName ~ remainder`,
				remainder
			);
		} else {
	...
	function parseQuoteAuthorTitle(workingQuoteObject, separatorValue) {
		...
		separatorValue = findNextSeparatingCharacter(item);
		console.log(
			`🚀 ~ file: parseQuotes.svelte ~ line 261 ~ parseQuoteAuthorTitle ~ separatorValue`,
			separatorValue
		);
		nextPart = nameNextPartOfQuote(item, separatorValue);
		workingQuoteObject['nextPart'] = nextPart;
		console.log(
			`🚀 ~ file: parseQuotes.svelte ~ line 267 ~ parseQuoteAuthorTitle ~ nextPart`,
			nextPart
		);

		if (nextPart) {
			title = Array.from(item)
				.splice(1, separatorValue - 1)
				.join(String())
				.trim();
		...
		workingQuoteObject['remainder'] = remainder;
		console.log(
			`🚀 ~ file: parseQuotes.svelte ~ line 281 ~ parseQuoteAuthorTitle ~ workingQuoteObject\n\n`,
			workingQuoteObject,
			`\n\n`
		);
		return workingQuoteObject;
	...
	function parseQuoteDate(workingQuoteObject, separatorValue) {
		console.log(
			`🚀 ~ file: parseQuotes.svelte ~ line 281 ~ parseQuoteDate ~ separatorValue`,
			separatorValue
		);
		console.log(
			'\x1b[31m%s\x1b[0m',
